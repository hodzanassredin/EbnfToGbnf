...
TERMINALS = ["ident", "number", "character", "string"]
    ...
    for child in factor_node.children:
        if isinstance(child, Tree):
            ...
            if child.value in rule_names:
                result.append(name_fix(child.value))
...
def rebuild_rule(rule_node: Tree, rule_names: List[str]) -> str:
    ...
    return f"{name_fix(id_node.value)}: {rebuild_expression(expression_node, rule_names)}"
